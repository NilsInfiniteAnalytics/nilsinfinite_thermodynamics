...
impl ThermodynamicParameterSet {
    pub fn new() -> Self {
        Self {
            t_0: STANDARD_TEMPERATURE,
            mslp: STANDARD_PRESSURE,
    ...
    pub fn gas_constant_air(&self, q: &MoistAirPhasePartition) -> f64 {
        let r_d = self.r_d();
        let molmass_ratio = self.molmass_ratio();
        r_d * (1.0 + (molmass_ratio - 1.0) * q.total - molmass_ratio * (q.liquid + q.ice))
    }
}

pub struct MoistAirPhasePartition {
    pub total: f64,
    pub liquid: f64,
    pub vapor: f64,
    pub ice: f64,
...
impl MoistAirPhasePartition {
    pub fn new(total: f64, liquid: f64, vapor: f64, ice: f64, dry: f64) -> Self {
        Self {
            total,
            liquid,
            vapor,
...
// Methods
/* 
* cp_m(parameter_set, phase_partition)
* The isobaric specific heat capacity of moist air given
* -parameter_set: the thermodynamic parameter set containing the specific heats
* -phase_partition: the phase partition containing the mass fractions for the moist air
*/
pub fn cp_m(
    parameter_set: &ThermodynamicParameterSet, 
    phase_partition: &MoistAirPhasePartition
) -> f64 {
    let cp_d = parameter_set.cp_d();
    let cp_v = parameter_set.cp_v;
    let cp_l = parameter_set.cp_l;
    let cp_i = parameter_set.cp_i;
    return cp_d + (cp_v - cp_d) * phase_partition.total
                + (cp_l - cp_v) * phase_partition.liquid
                + (cp_i - cp_v) * phase_partition.ice
}
